...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor,
  ...
  with tf.name_scope(layer_name):
    with tf.name_scope('weights'):
      ...
      #Adding Dropout 
      keep_prob = 0.5
      dense = tf.layers.dense(inputs=resized_input_tensor, units=1024, activation=tf.nn.softmax)
      #dropout = tf.layers.dropout(inputs=dense, rate=keep_prob, training=mode == tf.estimator.ModeKeys.TRAIN)
      dropout = tf.layers.dropout(inputs=dense, rate=keep_prob)
      logits = tf.layers.dense(inputs=dropout, units=10)
      with tf.name_scope('dropout'):
        keep_prob = tf.placeholder(tf.float32)
        #tf.scalar_summary('dropout_keep_probability', keep_prob)
        tf.summary.scalar('dropout_keep_probability', keep_prob)      # changed 'scalar_summary' to 'summary.scalar' 
        drop = tf.nn.dropout(resized_input_tensor, keep_prob)   # assigned input_tensor with resized_input_tensor
